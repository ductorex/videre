...
- `Window`: Main entry point. Takes an `AbstractBackend` (default `PygameBackend()`) and asks it for a renderer + a windowing — exposed as `Window.renderer` / `Window.windowing` — plus a `TaskManager`. `Window.run()` delegates to `windowing.run()`, which drives the event loop at 60 FPS (`WINDOW_FPS`); `Window._refresh` hands the freshly built root drawer to `renderer.render_drawer(drawer)` (the renderer owns the paint target and the frame-to-frame skip). Provides `call_later`/`call_async`/`call_now` for callback scheduling with different timing guarantees, and `text_rendering(...)` (builds a `TextRendering`).
