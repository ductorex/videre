...
- **Property tracking via `__wprops__`**: Each widget class declares properties in a `__wprops__` tuple (or set). Values are stored in the `_props` dict. Access via `_get_wprop()`/`_set_wprop()`. `_has_wprop()` walks the MRO to find properties across the inheritance hierarchy. Change detection is **push-based**: a mutation (`_set_wprop`/`_set_wprops`/`update`/`_set_transient`) flips a `_dirty` flag and bubbles it up the parent chain (`_mark_dirty`, stopping at the first already-dirty ancestor), so `has_changed()` is an O(1) flag read — no per-frame tree walk. (`_mark_ancestors_dirty` dirties only the ancestors, used by the animation re-arm — see Windowing.)
- **Rendering pipeline**: `render(window, width, height)` checks three conditions: `_surface is None`, `_old_update != (window, width, height)` (context changed), or `has_changed()` (properties changed). Only when dirty does it call `draw()`, which returns a `Drawer` (a command IR — see "Rendering backend"). After rendering it clears `_dirty` and `_transient_state`. Each widget caches its `_surface` (a `Drawer`) for reuse when clean. `draw()` records commands via `Drawing.*` / `Drawer` methods — it never allocates a surface or imports `pygame`; the backend rasterizes the `Drawer` afterwards.
- **Transient state**: `update()` sets `_transient_state["redraw"] = True` **and** marks the widget dirty, to force the next render without changing actual properties. Other transient flags must be set via `_set_transient(key, value)` so they bubble too — a raw `_transient_state[...] =` write would no longer trigger a redraw now that detection is push-based. Transient state is cleared after each render cycle.
...
- **`AbstractLayout`**: Base for all containers. Holds `_controls` wprop. `has_changed()` is inherited as an O(1) `_dirty` read (a dirty child has already bubbled the flag up — no recursion); `flush_changes()` still recurses to children. Defines `__size__` (enforces exact child count if set) and `__capture_mouse__` (if True, captures mouse even outside children).
...
- `Window`: Main entry point. Takes an `AbstractBackend` (default `PygameBackend()`) and asks it for a renderer + a windowing — exposed as `Window.renderer` / `Window.windowing` — plus a `TaskManager`. `Window.run()` delegates to `windowing.run()`, which drives the event loop at 60 FPS (`WINDOW_FPS`); `Window._refresh` hands the freshly built root drawer to `renderer.render_drawer(drawer)` (the renderer owns the paint target and the frame-to-frame skip). Provides `call_later`/`call_async`/`call_now` for callback scheduling with different timing guarantees, and `text_rendering(...)` (builds a `TextRendering`). Frame-driven widgets (`Animator`, `AbstractAnimation`) tick from their own `render()` and call `Window.request_frame(self)` each frame; `_step` drains those requests after `_refresh` (`_drain_frame_requests`), dirtying **only their ancestors** so the tree descends to them next frame while they redraw their own surface only when their content changes. The request set is rebuilt every frame, so a widget removed from the tree simply stops re-requesting — no registry, no unregister.
