"""Re-run the whole `tests/videre_tests` suite against the SHAPED text renderer.

POC harness. `pytest_collect_file` (triggered by the empty `_mirror.py`) builds
one virtual module per `tests/videre_tests/test_*.py`, anchored under this
...
class _MirrorModule(pytest.Module):
    ...
    here) but whose code is the real source module from `tests.videre_tests`."""
...
class _MirrorFile(pytest.File):
    def collect(self):
        ...
        src_dir = self.path.parents[2] / "videre_tests"  # tests/videre_tests
