...
def _surface_height(fake_win, text: str, **kwargs) -> int:
    return (
        ShapedTextRendering(fake_win.backend, **kwargs)
...
def test_bold_renders_wider_than_regular(fake_win) -> None:
    text = "Hello bold"
    s_reg = ShapedTextRendering(fake_win.backend, size=24).render_text(
        text, color=Color(0, 0, 0)
    )[1]
    s_bold = ShapedTextRendering(fake_win.backend, size=24, bold=True).render_text(
        text, color=Color(0, 0, 0)
    )[1]
...
def test_italic_does_not_crash_and_is_not_identical(fake_win) -> None:
    ...
    s_it = ShapedTextRendering(fake_win.backend, size=24, italic=True).render_text(
        text, color=Color(0, 0, 0)
    )[1]
...
def test_render_char_empty_returns_zero_size(fake_win) -> None:
    s = ShapedTextRendering(fake_win.backend, size=16).render_char("")
...
def test_render_char_returns_tight_glyph_bitmap(fake_win) -> None:
    ...
    s = ShapedTextRendering(fake_win.backend, size=16).render_char("A")
...
def test_render_char_widget_symbols_have_pixels(fake_win, char: str) -> None:
    ...
    s = ShapedTextRendering(fake_win.backend, size=22).render_char(char)
    w, h = s.get_width(), s.get_height()
    assert w > 0 and h > 0
    arr = pixels_alpha(s)
