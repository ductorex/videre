...
def test_compute_key_x_visual_right_in_pure_rtl_decreases_source(fake_win) -> None:
...
def test_visual_state_at_pixel_clicks_at_left_edge_yields_first_position(
    fake_win,
) -> None:
...
def test_visual_state_at_pixel_clicks_inside_rtl_run_picks_visual_glyph(
    fake_win,
) -> None:
...
def test_ctrl_right_visual_jumps_to_next_word_in_ltr_then_rtl(fake_win) -> None:
...
def test_hello_plus_turkish_ottoman_backward_caret_pixel_is_monotonic(fake_win) -> None:
...
def test_visual_range_to_source_set_partial_rtl_picks_visually_adjacent(
    fake_win,
) -> None:
...
def test_visual_selection_rects_across_bidi_boundary_is_single_ribbon(fake_win) -> None:
...
def test_total_visual_count_matches_total_items(fake_win) -> None:
...
def test_delete_visual_selection_across_ltr_rtl_boundary(fake_win) -> None:
...
def test_copy_visual_selection_preserves_source_order(fake_win) -> None:
