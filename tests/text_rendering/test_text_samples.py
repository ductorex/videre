...
These exercise `TextRendering` **standalone** — instantiated directly
on the backend, not through the `Text` widget (which still routes to the
legacy `PygameTextRendering`). The point is to pin the *new* text_rendering stack
end to end (segmentation -> HarfBuzz text_rendering -> wrap -> bidi reorder ->
