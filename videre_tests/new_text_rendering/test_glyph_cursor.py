...
def _render(text: str, **kw) -> "tuple":
    return ShapedTextRendering(size=16).render_text(text, color=Color(0, 0, 0), **kw)


# -- glyph_caret_pixel ------------------------------------------------------


def test_glyph_caret_pixel_empty_layout_returns_origin() -> None:
    out, _ = _render("")
    caret = out.glyph_caret_pixel(GlyphCursor(0, 0))
    assert caret.x == 0


def test_glyph_caret_pixel_ltr_first_is_left_edge() -> None:
    """First glyph cursor sits at the visual left edge of items[0]."""
    out, _ = _render("hello")
    assert out.glyph_caret_pixel(GlyphCursor(0, 0)).x == 0


def test_glyph_caret_pixel_ltr_last_is_right_edge_of_last_item() -> None:
    out, _ = _render("hello")
...
def test_glyph_caret_pixel_advances_left_to_right_along_glyph_index() -> None:
...
def test_pixel_to_glyph_at_origin_yields_first_position() -> None:
    out, _ = _render("hello")
    assert out.pixel_to_glyph(0, 0) == GlyphCursor(0, 0)


def test_pixel_to_glyph_far_right_yields_last_position() -> None:
    out, rendered = _render("hello")
    n_items = len(out.line_layouts[0].items)
    cursor = out.pixel_to_glyph(rendered.surface.get_width() + 100, 0)
    assert cursor == GlyphCursor(0, n_items)


def test_pixel_to_glyph_round_trip_ltr() -> None:
...
def test_pixel_to_glyph_round_trip_rtl() -> None:
...
def test_pixel_to_glyph_round_trip_mixed_ltr_rtl_ltr() -> None:
...
def test_glyph_to_source_ltr() -> None:
...
def test_glyph_to_source_rtl_pure() -> None:
...
def test_source_to_glyph_ltr_round_trip() -> None:
    text = "hello"
    out, _ = _render(text)
...
def test_source_to_glyph_rtl_round_trip() -> None:
...
def test_next_glyph_advances_within_line() -> None:
    out, _ = _render("hello")
    cur = GlyphCursor(0, 0)
...
def test_next_glyph_clamps_at_end_of_document() -> None:
    out, _ = _render("hi")
...
def test_prev_glyph_recedes_within_line() -> None:
    out, _ = _render("hello")
    n = len(out.line_layouts[0].items)
    cur = GlyphCursor(0, n)
...
def test_prev_glyph_clamps_at_start_of_document() -> None:
    out, _ = _render("hi")
    assert out.prev_glyph(GlyphCursor(0, 0)) == GlyphCursor(0, 0)


def test_next_glyph_wraps_to_next_line() -> None:
...
def test_prev_glyph_wraps_to_previous_line() -> None:
    out, _ = _render("alpha\nbeta")
    n_items_line0 = len(out.line_layouts[0].items)
...
def test_next_glyph_in_rtl_run_decreases_source_position() -> None:
...
def test_glyph_navigation_traverses_mixed_line_left_to_right() -> None:
