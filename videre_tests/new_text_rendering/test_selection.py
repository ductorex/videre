...
The selection is expressed in logical source positions (matching what
`TextSequence` indexes into) and produces a translucent blue rectangle
...
def _highlight_x_range(s_sel: pygame.Surface, s_no: pygame.Surface) -> tuple[int, int]:
    ...
    """
    arr_diff = sa.pixels_blue(s_sel).astype(int) - sa.pixels_blue(s_no).astype(int)
...
def test_selection_none_is_unchanged() -> None:
    ...
    s_none = r.render_text(text, (0, 0, 0), selection=None)[1].surface
...
def test_selection_empty_range_is_noop() -> None:
    """A degenerate range `[k, k)` selects nothing."""
    r = ShapedTextRendering(size=16)
    text = "Hello world"
    s_no = r.render_text(text, (0, 0, 0))[1].surface
    s_empty = r.render_text(text, (0, 0, 0), selection=(3, 3))[1].surface
    assert (sa.pixels_blue(s_no) == sa.pixels_blue(s_empty)).all()


def test_selection_introduces_blue_band() -> None:
    """A non-empty selection must paint a translucent blue band."""
    r = ShapedTextRendering(size=16)
    text = "Hello world"
    s_no = r.render_text(text, (0, 0, 0))[1].surface
    s_sel = r.render_text(text, (0, 0, 0), selection=(1, 5))[1].surface
    lo, hi = _highlight_x_range(s_sel, s_no)
    assert lo > 0 and hi > lo


def test_selection_keeps_glyph_color() -> None:
    ...
    text = "Hello"
    s_no = r.render_text(text, (0, 0, 0))[1].surface
    s_sel = r.render_text(text, (0, 0, 0), selection=(0, 5))[1].surface
    ...
    r_sel = sa.pixels_red(s_sel)
    g_sel = sa.pixels_green(s_sel)
    if fully_opaque_no.any():
...
def test_selection_starts_after_first_char() -> None:
...
def test_selection_full_text_covers_everything() -> None:
    ...
    s_no = r.render_text(text, (0, 0, 0))[1].surface
    n = len(text)
    s_sel = r.render_text(text, (0, 0, 0), selection=(0, n))[1].surface
    lo, hi = _highlight_x_range(s_sel, s_no)
...
def test_selection_across_wrapped_lines() -> None:
    """Selection spanning a wrap break must highlight on both lines."""
    r = ShapedTextRendering(size=16)
    text = "alpha beta gamma delta"
    width = 80
    s_no = r.render_text(text, (0, 0, 0), width=width, wrap_words=True)[1].surface
    s_sel = r.render_text(
        text, (0, 0, 0), width=width, wrap_words=True, selection=(0, len(text))
    )[1].surface
    diff = sa.pixels_blue(s_sel).astype(int) - sa.pixels_blue(s_no).astype(int)
...
def test_selection_across_paragraph_break() -> None:
