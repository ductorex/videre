...
def _items_of(text: str) -> list:
    out, _ = ShapedTextRendering(size=16).render_text(text, color=Color(0, 0, 0))
    assert len(out.line_layouts) == 1
    return list(out.line_layouts[0].items)


def test_render_text_items_are_in_visual_pixel_order() -> None:
...
def test_render_text_rtl_context_inverts_run_order_visually() -> None:
...
def test_render_text_ltr_context_with_rtl_word_keeps_run_order() -> None:
...
def test_render_text_pure_ltr_round_trips_unchanged_by_l2() -> None:
...
def test_render_text_arabic_internal_carets_move_right_to_left() -> None:
