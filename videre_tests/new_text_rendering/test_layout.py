...
def test_render_text_returns_shaped_rendered_text() -> None:
    ...
    out, rendered = r.render_text("Hello", (0, 0, 0))
...
def test_font_metrics_match_constructor() -> None:
    ...
    out, _ = r.render_text("Hello", (0, 0, 0))
    fm = out.font_metrics
...
def test_empty_text_reserves_one_empty_line_layout() -> None:
...
def test_pos_to_pixel_last_position_is_right_edge() -> None:
    r = ShapedTextRendering(size=16)
    text = "Hello"
    out, rendered = r.render_text(text, (0, 0, 0))
...
def test_pos_to_pixel_middle_position_advances_monotonically() -> None:
    """Caret x must increase as pos goes from 0 to len(text)."""
    r = ShapedTextRendering(size=16)
    text = "Hello"
    out, _ = r.render_text(text, (0, 0, 0))
...
def test_pos_to_pixel_aligned_center_shifts_caret() -> None:
    """CENTER alignment moves the line's left edge to the right;
    pos_to_pixel(0) must reflect that x_offset, not be 0."""
    r = ShapedTextRendering(size=16)
    out, _ = r.render_text(
        "Hello", (0, 0, 0), width=200, wrap_words=True, align=TextAlign.CENTER
    )
    caret = out.pos_to_pixel(0)
    assert caret.x > 0
...
def test_pixel_to_pos_below_clamps_to_last_line() -> None:
...
def test_round_trip_at_cluster_boundaries() -> None:
...
def test_pos_to_pixel_in_inter_word_gap() -> None:
