...
from videre.colors import Colors, parse_color, stringify_color, Color
...
def test_parse_color():
    ...
    assert parse_color((1, 2, 3, 227)) == Color(1, 2, 3, 227)
    assert parse_color([1, 2, 3, 227]) == Color(1, 2, 3, 227)

    color = Color(4, 5, 7, 11)
    assert parse_color(color) is color

    assert parse_color("green") == Colors.green

    assert parse_color("#2bffea") == Color(0x2B, 0xFF, 0xEA)
    assert parse_color("#2bffeadc") == Color(0x2B, 0xFF, 0xEA, 0xDC)
    assert parse_color("#2bffe") == Color(0x2, 0xBF, 0xFE)
    assert parse_color("#12c") == Color(0, 0x1, 0x2C)
    assert parse_color("#c") == Color(0, 0, 0xC)

    assert parse_color(None) == Colors.transparent


def test_stringify_color():
    assert stringify_color(Color(0, 0, 0, 0)) == "transparent"
    assert stringify_color(Color(0, 0, 0, 255)) == "black"
    assert stringify_color(Color(255, 255, 255, 255)) == "white"
    assert stringify_color(Color(255, 255, 255)) == "white"
    assert stringify_color(Color(255, 255, 254)) == "#fffffe"
    assert stringify_color(Color(255, 0xA4, 0xB9)) == "#ffa4b9"
