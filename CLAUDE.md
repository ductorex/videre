...
Widgets call `Window.text_rendering(size, strong, italic, underline, height_delta)` → `backend.text_rendering(...)` → an `AbstractTextRendering`, whose `render_text(...)` returns `(TextRenderingResult, Rendering)` and `render_char(...)` returns a `Rendering`. `TextRenderingResult` (`core/rendering_result.py`) is the cursor/hit-test contract `widgets/textinput` relies on — bidi-aware visual navigation through an opaque `CursorState`, plus surface-less sizing (`get_width/height`) for the `Drawer`/`text_sizing` path.

Two implementations both satisfy `AbstractTextRendering`:

1. **Legacy pygame — currently LIVE.** `pygame_backend/text_rendering.py::PygameTextRendering`, on `pygame.freetype` + `PygameFontFactory`. No bidi (visual order = source order). `PygameBackend.text_rendering()` always returns this today.
...
2. **Shaping pipeline (`videre/core/shaping/`) — tested, NOT live.** A from-scratch flat Unicode stack on `AbstractBackend` primitives only. Flow: `partition_text → shape_line → wrap → reorder → render`, exposed by `text_rendering.py::ShapedTextRendering`. Package layout mirrors the data flow: `text_partition/` = logical segmentation; root = shaped/visual model + shaping + rasterization + painting; `rendering/` = line assembly (wrap, space policy, reorder, caret). **Each module has a dense docstring — read it for the detail.** The non-obvious cross-cutting facts:
   - **`core/text_editing.py`** — the `EditUnit` model (immutable UAX#29 grapheme ranges, classified, never filtered) shared by the pipeline *and* `TextInput`: backspace/delete/selection/insertion all work at edit-unit granularity.
   ...
   - **Wiring** — no env flags. `ShapedTextRendering(subpixel=…)` threads sub-pixel positioning; `Window.__init__(handle_text_sub_pixels)` stores `Window._subpixel` but **nothing reads it yet** — it is the intended hook to make the shaped renderer live. Bench: `tools/bench_text_rendering.py`.
   ...
   Removed (don't look for them — see git): `python-bidi`, the old renderable model, the per-run shaping pipeline, and the transitional `new_text_partition/` package (the 2026-06 reorg split it into `text_partition/` + `rendering/` + root modules; `env.py` is gone).
