...
Source positions follow a printable-source convention (printable
...
class _LineItem:
    ...
    document (the same printable-source convention), with
...
@dataclass(slots=True, frozen=True)
class _ShapedCursorState(CursorState):
...
@dataclass(slots=True, frozen=True)
class ShapedRenderedText(TextRenderingResult):
    ...
    def next_visual(self, state: CursorState) -> CursorState:
        return self._make_state(self.next_glyph(cast(_ShapedCursorState, state).glyph))

    def prev_visual(self, state: CursorState) -> CursorState:
        return self._make_state(self.prev_glyph(cast(_ShapedCursorState, state).glyph))
    ...
    def next_visual_word(self, state: CursorState, text: str) -> CursorState:
        ...
        glyph = cast(_ShapedCursorState, state).glyph
        while True:
