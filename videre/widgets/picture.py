...
class Picture(Widget):
    ...
    Sizes are logical pixels. Never pre-resize a bitmap to its display
    size: the renderer resamples the native bitmap once, straight to device
    pixels, so the picture stays as sharp as the source allows."""
    ...
    def _src_to_surface(self, window: "Window") -> Drawer | None:
        ...
        try:
            ...
            return window.drawing.image(image)
    ...
    def draw(
        ...
        # Always through `smoothscale`, even at natural size: an image
        # drawer is native raw pixels; displaying it at a logical size is
        # the drawing's job (a no-op when the sizes already match).
        return window.drawing.smoothscale(surface, view_w, view_h)
